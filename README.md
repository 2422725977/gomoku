...
见 **[docs/submission/](docs/submission/)**：

| 文件 | 说明 |
|---|---|
| `demo.mp4` | 功能演示视频，2 分 43 秒 / 1000×760 / 30fps / 2.1 MB |
| `封面_338x210.png` / `.jpg` | 作品封面（表单建议尺寸 338×210） |
| `封面_1014x630.png` | 封面 3 倍图，备用 |
| `提交文案.md` | 作品标题与简介（含长短两版），可直接复制粘贴 |

> 演示视频用 Godot 的 **Movie Maker 模式**逐帧录制（`--write-movie --fixed-fps 30`）
> 再用 ffmpeg 编码，分镜脚本见 git 历史中的 `tools/demo.gd`。
> 因 Godot 帧缓冲不含操作系统光标，录制时自绘了箭头光标与点击涟漪。
